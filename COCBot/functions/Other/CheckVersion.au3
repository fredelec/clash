...
Func CheckVersion()
	If $ichkVersion = 1 Then
		CheckVersionHTML()
		If $lastversion = "" Then
			SetLog("CANNOT OBTAIN PRODUCT VERSION AT THIS TIME", $COLOR_ORANGE)
		ElseIf VersionNumFromVersionTXT($sBotVersion) < VersionNumFromVersionTXT($lastversion) Then
			...
			SetLog("PLEASE DOWNLOAD THE LATEST(" & $lastversion & ") ASK TO FREDELEC               ", $COLOR_RED);HERE page onlie for download
			SetLog(" ")
			_PrintLogVersion($oldversmessage)
		ElseIf VersionNumFromVersionTXT($sBotVersion) > VersionNumFromVersionTXT($lastversion) Then
			SetLog("CONGRATULATION ARE USING AN UPDATED VERSION OF 'I,ROBOT'", $COLOR_GREEN)
			SetLog("YOUR VERSION: " & $sBotVersion, $COLOR_GREEN)
			;SetLog("OFFICIAL VERSION: " & $lastversion, $COLOR_GREEN)
			SetLog(" ")
		Else
			SetLog("WELCOME CAPO", $COLOR_GREEN)
			SetLog("YOU HAVE THE LATEST VERSION OF THE 'I,ROBOT'", $COLOR_GREEN)
			SetLog(" ")
...
Func CheckVersionHTML()
	Local $versionfile = @ScriptDir & "\message_welcome.txt"
...
;~ 	Else
	...
	;	$hDownload = InetGet("https://raw.githubusercontent.com/MyBotRun/MyBot/master/LastVersion.txt", $versionfile)
		$hDownload = InetGet("https://raw.githubusercontent.com/fredelec/clash/master/message_welcome.txt", $versionfile)
...
;~ 	EndIf
	...
	If FileExists($versionfile) Then
		...
		;look for localized messages for the new and old versions
 		Local $versionfilelocalized = @ScriptDir & "\fredversion_" & $sLanguage & ".txt"; declaration of variable versionfilelocalized
;~  		If FileExists(@ScriptDir & "\fredversion_" & $sLanguage & ".txt") Then
;~  			FileCopy(@ScriptDir & "\fredversion_" & $sLanguage & ".txt", $versionfilelocalized, 1)
;~  		Else
...
;			$hDownload = InetGet("https://raw.githubusercontent.com/MyBotRun/MyBot/master/LastVersion_" & $sLanguage & ".txt", $versionfilelocalized)
			$hDownload = InetGet("https://raw.githubusercontent.com/fredelec/clash/master/fredversion_" & $sLanguage & ".txt", $versionfilelocalized)
